...
PPEs have played an important role in assessing projection uncertainty in climate models \cite{sanderson2008,hawkins2019,yamazaki2021,peatier2022,tett2022}.
PPEs also form a basis for automated model calibration through, for example, history matching \cite{williamson2013} or Bayesian uncertainty quantification \cite{cleary2021}.
...
\caption{Global annual gross primary production (GPP) and net ecosystem production (NEP) across the PPE within each of our six forcing scenarios. Circles are drawn for 5 and 0.1 PgC bins respectively, with the circle size proportional to the number of simulations falling in that bin. In each case the largest circle is centered at the default simulation.}
\label{fig:violins}
\end{figure}
